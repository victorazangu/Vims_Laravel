...
<div class="flex items-center justify-between pb-4 bg-white dark:bg-gray-900">
    ...
    <div class="relative">
        ...
        <input type="text" id="table-search-users" class="block p-2 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg w-80 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search for lecturer">
    </div>
</div>
@endsection
@section('add-button')
            ...
            <a href="{{ route('lecturer_show',$lecturer) }}">
                <li class="flex justify-between gap-x-6 py-5">
                    <div class="flex gap-x-4">
                        <img class="h-12 w-12 flex-none rounded-full bg-gray-50" src="{{ $lecturer->profile }}" alt="">
                        <div class="min-w-0 flex-auto">
                            <p class="text-sm font-semibold leading-6 text-gray-900">
                                {{ Str::title($lecturer->qualification) }}
                                {{ Str::title($lecturer->firstName) }}
                                {{ Str::title($lecturer->lastName) }}
                            </p>
                    ...
                    <div class=" sm:flex sm:flex-col sm:items-end">
                        <form action="{{ route('lecturer_delete',$lecturer) }}" method="POST">
                            @csrf
                            @method('DELETE')
